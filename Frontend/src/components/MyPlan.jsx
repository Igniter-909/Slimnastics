...
function MyPlan() {
  ...
  const filteredPlans = plans?.filter((plan) => plan.planId)
    ...
    <div className='p-4 w-full flex flex-col h-full rounded-xl border-2 border-red-500/40'>
      ...
      <div className='space-y-6'>
        {filteredPlans.map((plan , index) => (
          <div key={index} className='w-full rounded-xl border-2 border-red-500/40 p-4 md:p-6'>
            ...
            <ul className={`text-sm font-vazirmatn ${isDarkMode ? "text-white/30" : "text-black/30"} list-disc pl-5 space-y-1`}>
